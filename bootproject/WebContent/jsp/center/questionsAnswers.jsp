							...
							<div class="layui-form-item">
								...
								<div class="layui-input-block">
									<input type="text" id="question" name="question"  required
										lay-verify="required" placeholder="请输入问题" autocomplete="off"
										class="layui-input">
								</div>
							</div>
							 <div class="layui-form-item layui-form-text">
							    ...
							    <div class="layui-input-block">
							      <textarea id="answers" placeholder="请输入答案" required lay-verify="required" name="answers"  class="layui-textarea"></textarea>
							    </div>
							  </div>
	...
	<%}%>
		...
		<script>
			...
			$(function() {
				layui.use('table', function() {
					...
					table.render({
						elem : '#newslist',
						height : 312,
						url : '../questionAnswerss/findAllQuestionAnswers.action' //数据接口
						,
						page : true //开启分页
						,
						cols : [ [ //表头
					...
					form.on('submit(formAddCity)', function(data) {
						...
						$.ajax({
							url : '../questionAnswerss/addQuestionAnswers.action',
							data : data.field,
							type : 'POST',
							dataType : 'json',
							success : function(data) {
								if (data.code == 200) {
									layer.msg("新增问答成功", function() {
										// 获得frame索引
					...
					form.on('submit(formUpdateCity)', function(data) {
						...
						$.ajax({
							url : '../questionAnswerss/updateQuestionAnswers.action',
							data : data.field,
							type : 'POST',
							dataType : 'json',
							success : function(data) {
								if (data.code == 200) {
									layer.msg("更新问答成功", function() {
										// 获得frame索引
										var index = parent.layer
												.getFrameIndex(window.name);
										//关闭当前frame
			...
			function setFormValue(id) {
				$.ajax({
	                    type : 'POST',
	                    url : '../questionAnswerss/findQuestionAnswersById.action',
	                    data : {
	                        'id' : id
	                    },
	                    dataType : 'json',
	                    success : function(data) {
	                        if (data.code === 200) {
	                        	popForm.val("formTestFilter", {
	                                "id":data.rows.id,
	                                "question" : data.rows.question,
	                                "answers" : data.rows.answers
	                            });
			...
			function deleteNews(id) {
				$.ajax({
					type : 'POST',
					url : '../questionAnswerss/deleteQuestionAnswers.action',
					data : {
						'id' : id
					},
					dataType : 'json',
