						...
						<div class="layui-form-item">
							...
							<div class="layui-input-block">
								<input type="number" name="cost" required lay-verify="required"
									placeholder="请输入费用" autocomplete="off" class="layui-input">
							</div>
						</div>

						<div class="layui-form-item">
							<label class="layui-form-label">级别</label>
							<div class="layui-input-block" id="csotDiv"></div>
						</div>
		...
		$(function() {
			layui.use([ 'table', 'form', 'layer' ], function() {
				...
				table.render({
					elem : '#newslist',
					height : 312,
					url : '../costs/findAllCost.action' //数据接口
					,
					page : true //开启分页
					,
					cols : [ [ //表头
					{
						...
						title : '级别',
						width : 120,
						templet : '<div>{{Format(d.level.level)}}</div>'
					}, {
						title : '费用',
						width : 120,
						templet : '<div>{{createrFormat(d.cost)}}</div>'
					}, {
						fixed : 'right',
						width : 178,
						title : '操作',
				...
				form.on('submit(formUpdateSubject)', function(data) {
					...
					//   alert(formData);
					$.ajax({
						url : '../costs/updateCost.action',
						data : data.field,
						type : 'POST',
						dataType : 'json',
						success : function(data) {
							layer.msg("更新费用成功",function() {
                                // 获得frame索引
                                var index = parent.layer
                                        .getFrameIndex(window.name);
                                //关闭当前frame
			...
			$
					.ajax({
						...
						success : function(res) {
							if (res.status === 200) {
								var list = res.rows;
								var str = '<select name="lid" lay-verify="required" id="level">';
