...
<body>
						...
						<div class="layui-form-item" style="padding-left: 10px;">
							<div class="layui-input-block" id="addOperate">
								<button class="layui-btn layui-btn-normal" lay-submit
									lay-filter="formAddCategory">提交</button>
							...
							<div class="layui-input-block" id="updateOperate" style="display:none">
								<button class="layui-btn layui-btn-normal" lay-submit
									lay-filter="formUpdateCategory" >提交</button>
								<button type="reset" class="layui-btn layui-btn-primary">重置</button>
								<button class="layui-btn layui-btn-primary">关闭</button>
							</div>
						</div>
	...
	<script>
		...
		$(function() {
			layui.use('table', function() {
				...
				table.render({
					elem : '#newslist',
					height : 312,
					url : '../categorys/findAllCategory.action' //数据接口
					,
					page : true //开启分页
					,
					cols : [ [ //表头
				...
				form.on('submit(formAddCategory)', function (data) {
				    ...
				    $.ajax({
					 	  url : '../categorys/addCategory.action',
					 	  data : data.field,
					 	  type : 'POST',
					 	  dataType:'json',
				...
				form.on('submit(formUpdateCategory)', function (data) {
				    ...
				    $.ajax({
					 	  url : '../categorys/updateCategory.action',
					 	  data : data.field,
					 	  type : 'POST',
					 	  dataType:'json',
					 	  success : function(data) {
		...
		function deleteNews(id){
			$.ajax({
				  type: 'POST',
				  url: '../categorys/deleteCategory.action',
				  data: {'id':id},
				  dataType:'json' ,
				  success: function(data){
					  if(data.code === 200){
